...
namespace BlueAcorn\Core\Model\ContentVersion\Categorization;

use BlueAcorn\Core\Model\ContentVersion;
use BlueAcorn\Core\Model\ContentVersion\Categorization;
use BlueAcorn\Core\Model\ContentVersion\CategorizationFactory;
use BlueAcorn\Core\Model\ContentVersion\Entry;
use BlueAcorn\Core\Model\ResourceModel\ContentVersion\Collection;
use BlueAcorn\Core\Model\ResourceModel\ContentVersion\CollectionFactory;
                ...
                foreach ($entry->getDepends() as $dependType => $dependentEntries) {
                    foreach ($dependentEntries as $dependentEntryName) {
                        if (!isset($this->entryMap[$dependType][$dependentEntryName])) {
                            throw new LocalizedException(__('Content Version dependencies must exist as a blueacorn_content.xml configuration'));
                        }
                        $dependentEntry = $this->entryMap[$dependType][$dependentEntryName];
                        $this->processEntry($dependentEntry);
                    }
